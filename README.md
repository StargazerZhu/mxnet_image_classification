...
> Note: This project is from example codes of MXNet repository. I changed some codes in `fit.py` so that this 
project supports the newly released `gluon` package now. However, I found some bugs on `gluon.Trainer` when doing distributed
  training, especially for distributed asynchronous training. Therefore, I modified the Trainer class by creating a subclass 
  `RevisedTrainer` in the subfolder `common`. 


This folder contains examples for image classification. The goal of image
